...
if (process.env.RENDER) {
  ...
  setInterval(async () => {
    try {
      ...
      if (response.ok) console.log('✅ Self‑ping successful at', new Date().toISOString());
      else console.log('⚠️ Self‑ping returned status', response.status);
    } catch (err) { console.error('❌ Self‑ping failed:', err.message); }
  }, PING_INTERVAL);
  console.log('🔄 Self‑ping system activated (interval: 5 minutes)');
}

app.get('/api/health', (req, res) => { res.status(200).json({ status: 'ok', timestamp: new Date().toISOString(), environment: process.env.RENDER ? 'render' : 'local' }); });

const allowedOrigins = process.env.ALLOWED_ORIGINS ? process.env.ALLOWED_ORIGINS.split(',').map(origin => origin.trim()) : [process.env.CLIENT_URL_1, process.env.CLIENT_URL_2, process.env.FRONTEND_URL, process.env.ADMIN_URL, 'http://localhost:3000', 'http://localhost:5000', 'http://localhost:5173', 'http://127.0.0.1:5500'].filter(Boolean);

app.use(helmet({ crossOriginResourcePolicy: { policy: "cross-origin" }, crossOriginEmbedderPolicy: false }));
app.use(cors({ origin: function (origin, callback) { if (!origin) return callback(null, true); if (allowedOrigins.indexOf(origin) !== -1 || allowedOrigins.includes('*')) callback(null, true); else { console.log('🚫 Blocked origin:', origin); callback(new Error('Not allowed by CORS')); } }, credentials: true, methods: ['GET', 'POST', 'PUT', 'DELETE', 'OPTIONS'], allowedHeaders: ['Content-Type', 'Authorization'], optionsSuccessStatus: 200 }));
app.use(express.json({ limit: '50mb' }));
app.use(express.urlencoded({ extended: true, limit: '50mb' }));

const limiter = rateLimit({ windowMs: 15 * 60 * 1000, max: 500, message: 'Too many requests from this IP, please try again later.', standardHeaders: true, legacyHeaders: false, });
app.use('/api/', limiter);

cloudinary.config({ cloud_name: process.env.CLOUDINARY_CLOUD_NAME, api_key: process.env.CLOUDINARY_API_KEY, api_secret: process.env.CLOUDINARY_API_SECRET });

const storage = new CloudinaryStorage({ cloudinary: cloudinary, params: { folder: 'amayra', allowed_formats: ['jpg', 'jpeg', 'png', 'webp', 'gif'], transformation: [{ width: 1200, height: 1200, crop: 'limit' }] } });
const upload = multer({ storage, limits: { fileSize: 10 * 1024 * 1024 } });

const connectWithRetry = () => { mongoose.connect(process.env.MONGODB_URI, { serverSelectionTimeoutMS: 15000, socketTimeoutMS: 45000 }).then(() => console.log('✅ MongoDB Connected')).catch(err => { console.error('❌ MongoDB Connection Error:', err); console.log('🔄 Retrying connection in 5 seconds...'); setTimeout(connectWithRetry, 5000); }); };
connectWithRetry();
...
const auth = async (req, res, next) => { try { const token = req.header('Authorization')?.replace('Bearer ', ''); if (!token) throw new Error(); const decoded = jwt.verify(token, process.env.JWT_SECRET); const admin = await Admin.findById(decoded.id); if (!admin) throw new Error(); req.admin = admin; next(); } catch (error) { res.status(401).json({ error: 'Please authenticate' }); } };

async function initializeDatabase() {
  try {
    const adminExists = await Admin.findOne({ username: process.env.ADMIN_USERNAME });
    if (!adminExists) { const hashedPassword = await bcrypt.hash(process.env.ADMIN_PASSWORD, 10); await Admin.create({ username: process.env.ADMIN_USERNAME, password: hashedPassword, name: 'Super Admin' }); console.log('✅ Default admin created'); }
    const settingsExists = await Settings.findOne();
    if (!settingsExists) { await Settings.create({}); console.log('✅ Default settings created'); }
    const categoriesExist = await Category.countDocuments();
    if (categoriesExist === 0) { const defaultCategories = [ { name: 'formal', slug: 'formal', banglaName: 'ফরমাল', order: 1, icon: 'fa-user-tie' }, { name: 'casual', slug: 'casual', banglaName: 'ক্যাজুয়াল', order: 2, icon: 'fa-shirt' }, { name: 'premium', slug: 'premium', banglaName: 'প্রিমিয়াম', order: 3, icon: 'fa-crown' } ]; await Category.insertMany(defaultCategories); console.log('✅ Default categories created'); }
    const slidersExist = await Slider.countDocuments();
    if (slidersExist === 0) { const defaultSliders = [ { title: 'প্রিমিয়াম কালেকশন', subtitle: 'আড়ম্বরপূর্ণ ডিজাইন, অনন্য গুণগত মান', badge: 'সীমিত অফার', badgeColor: '#1877F2', imageUrl: 'https://images.unsplash.com/photo-1598033128083-2f68a5d9b1b1?w=1200&q=85', productId: 1, order: 1 }, { title: 'হোয়াইট প্রিমিয়াম', subtitle: 'হাতের ছোঁয়ায় প্রিমিয়াম সুতি', badge: 'বেস্টসেলার', badgeColor: '#FF7A00', imageUrl: 'https://images.unsplash.com/photo-1602810316693-3667c854239a?w=1200&q=85', productId: 2, order: 2 }, { title: 'ডেনিম প্যাচ', subtitle: 'হাতের কাজ করা লিমিটেড এডিশন', badge: 'এক্সক্লুসিভ', badgeColor: '#1877F2', imageUrl: 'https://images.unsplash.com/photo-1588359348347-9bc6cbbb689c?w=1200&q=85', productId: 5, order: 3 } ]; await Slider.insertMany(defaultSliders); console.log('✅ Default sliders created'); }
  } catch (error) { console.error('Database initialization error:', error); }
}

app.post('/api/admin/login', [ body('username').notEmpty(), body('password').notEmpty() ], validate, async (req, res) => {
  try { const { username, password } = req.body; const admin = await Admin.findOne({ username }); if (!admin) return res.status(401).json({ error: 'Invalid credentials' }); const isMatch = await bcrypt.compare(password, admin.password); if (!isMatch) return res.status(401).json({ error: 'Invalid credentials' }); admin.lastLogin = new Date(); await admin.save(); const token = jwt.sign({ id: admin._id, username: admin.username, name: admin.name }, process.env.JWT_SECRET, { expiresIn: '30d' }); res.json({ token, admin: { username: admin.username, name: admin.name, id: admin._id } }); } catch (error) { res.status(500).json({ error: error.message }); }
});

app.get('/api/admin/verify', auth, async (req, res) => { try { const admin = await Admin.findById(req.admin._id).select('-password'); res.json(admin); } catch (error) { res.status(500).json({ error: error.message }); } });
app.post('/api/admin/change-password', auth, async (req, res) => {
  try { const { currentPassword, newPassword } = req.body; const admin = await Admin.findById(req.admin._id); const isMatch = await bcrypt.compare(currentPassword, admin.password); if (!isMatch) return res.status(400).json({ error: 'Current password is incorrect' }); admin.password = await bcrypt.hash(newPassword, 10); await admin.save(); res.json({ message: 'Password changed successfully' }); } catch (error) { res.status(500).json({ error: error.message }); }
});

app.get('/api/categories', async (req, res) => { try { const categories = await Category.find({ active: true }).sort('order'); res.json(categories); } catch (error) { res.status(500).json({ error: error.message }); } });
app.get('/api/sliders', async (req, res) => { try { const sliders = await Slider.find({ active: true }).sort('order'); res.json(sliders); } catch (error) { res.status(500).json({ error: error.message }); } });
...
app.get('/api/admin/categories', auth, async (req, res) => { try { const categories = await Category.find().sort('order'); res.json(categories); } catch (error) { res.status(500).json({ error: error.message }); } });
app.get('/api/admin/categories/all', auth, async (req, res) => { try { const categories = await Category.find().sort('order'); res.json(categories); } catch (error) { res.status(500).json({ error: error.message }); } });
app.post('/api/admin/categories', auth, async (req, res) => {
  try { const { name, banglaName, description, order, active, icon } = req.body; const slug = name.toLowerCase().replace(/\s+/g, '-'); const existing = await Category.findOne({ slug }); if (existing) return res.status(400).json({ error: 'Category slug already exists' }); const category = new Category({ name, slug, banglaName, description, icon: icon || 'fa-tags', order: order || 0, active: active !== false }); await category.save(); res.status(201).json(category); } catch (error) { res.status(500).json({ error: error.message }); }
});
app.put('/api/admin/categories/:id', auth, async (req, res) => { try { const { name, banglaName, description, order, active, icon } = req.body; const slug = name.toLowerCase().replace(/\s+/g, '-'); const category = await Category.findByIdAndUpdate(req.params.id, { name, slug, banglaName, description, icon, order, active }, { new: true }); res.json(category); } catch (error) { res.status(500).json({ error: error.message }); } });
app.delete('/api/admin/categories/:id', auth, async (req, res) => { try { const products = await Product.findOne({ category: req.params.id }); if (products) return res.status(400).json({ error: 'Cannot delete category with products' }); await Category.findByIdAndDelete(req.params.id); res.json({ message: 'Category deleted successfully' }); } catch (error) { res.status(500).json({ error: error.message }); } });

app.get('/api/admin/sliders', auth, async (req, res) => { try { const sliders = await Slider.find().sort('order'); res.json(sliders); } catch (error) { res.status(500).json({ error: error.message }); } });
app.post('/api/admin/sliders', auth, upload.single('image'), async (req, res) => {
  try { const sliderData = JSON.parse(req.body.data || '{}'); if (sliderData.productId) { const product = await Product.findOne({ id: sliderData.productId }); if (!product) return res.status(400).json({ error: 'Invalid product ID' }); } if (!req.file) return res.status(400).json({ error: 'Image is required' }); const newSlider = new Slider({ title: sliderData.title, subtitle: sliderData.subtitle, badge: sliderData.badge, badgeColor: sliderData.badgeColor, buttonText: sliderData.buttonText, productId: sliderData.productId, order: sliderData.order || 0, active: sliderData.active === true || sliderData.active === 'true', imageUrl: req.file.path, publicId: req.file.filename }); await newSlider.save(); res.status(201).json(newSlider); } catch (error) { console.error('Error creating slider:', error); res.status(500).json({ error: error.message }); }
});
app.put('/api/admin/sliders/:id', auth, upload.single('image'), async (req, res) => {
  try { const sliderData = JSON.parse(req.body.data || '{}'); if (sliderData.productId) { const product = await Product.findOne({ id: sliderData.productId }); if (!product) return res.status(400).json({ error: 'Invalid product ID' }); } const updateData = { title: sliderData.title, subtitle: sliderData.subtitle, badge: sliderData.badge, badgeColor: sliderData.badgeColor, buttonText: sliderData.buttonText, productId: sliderData.productId, order: sliderData.order || 0, active: sliderData.active === true || sliderData.active === 'true' }; if (req.file) { updateData.imageUrl = req.file.path; updateData.publicId = req.file.filename; const oldSlider = await Slider.findById(req.params.id); if (oldSlider?.publicId) await cloudinary.uploader.destroy(oldSlider.publicId); } const slider = await Slider.findByIdAndUpdate(req.params.id, updateData, { new: true }); res.json(slider); } catch (error) { console.error('Error updating slider:', error); res.status(500).json({ error: error.message }); }
});
...
app.get('/api/admin/reviews', auth, async (req, res) => {
  try { const { status, page = 1, limit = 20 } = req.query; const query = {}; if (status && status !== 'all') query.status = status; const reviews = await Review.find(query).sort('-createdAt').limit(parseInt(limit)).skip((parseInt(page) - 1) * parseInt(limit)); const total = await Review.countDocuments(query); res.json({ reviews, totalPages: Math.ceil(total / limit), currentPage: parseInt(page), total }); } catch (error) { res.status(500).json({ error: error.message }); }
});
app.get('/api/admin/reviews/unread-count', auth, async (req, res) => { try { const count = await Review.countDocuments({ isRead: false }); res.json({ count }); } catch (error) { res.status(500).json({ error: error.message }); } });
app.get('/api/admin/reviews/:id', auth, async (req, res) => { try { const review = await Review.findById(req.params.id); if (!review) return res.status(404).json({ error: 'Review not found' }); await Review.findByIdAndUpdate(req.params.id, { isRead: true }); res.json(review); } catch (error) { res.status(500).json({ error: error.message }); } });
app.put('/api/admin/reviews/:id', auth, async (req, res) => { try { const { status, isFeatured } = req.body; const review = await Review.findByIdAndUpdate(req.params.id, { status, isFeatured, isRead: true }, { new: true }); res.json(review); } catch (error) { res.status(500).json({ error: error.message }); } });
app.delete('/api/admin/reviews/:id', auth, async (req, res) => { try { await Review.findByIdAndDelete(req.params.id); res.json({ message: 'Review deleted successfully' }); } catch (error) { res.status(500).json({ error: error.message }); } });

app.get('/api/admin/settings', auth, async (req, res) => { try { let settings = await Settings.findOne(); if (!settings) settings = await Settings.create({}); res.json(settings); } catch (error) { res.status(500).json({ error: error.message }); } });
app.put('/api/admin/settings', auth, async (req, res) => { try { const settings = await Settings.findOneAndUpdate({}, { ...req.body, updatedAt: Date.now() }, { new: true, upsert: true }); res.json(settings); } catch (error) { res.status(500).json({ error: error.message }); } });

app.get('/api/admin/dashboard', auth, async (req, res) => {
  try { const totalOrders = await Order.countDocuments(); const pendingOrders = await Order.countDocuments({ status: 'pending' }); const confirmedOrders = await Order.countDocuments({ status: 'confirmed' }); const processingOrders = await Order.countDocuments({ status: 'processing' }); const shippedOrders = await Order.countDocuments({ status: 'shipped' }); const deliveredOrders = await Order.countDocuments({ status: 'delivered' }); const cancelledOrders = await Order.countDocuments({ status: 'cancelled' }); const totalProducts = await Product.countDocuments(); const totalReviews = await Review.countDocuments(); const pendingReviews = await Review.countDocuments({ status: 'pending' }); const recentOrders = await Order.find().sort('-createdAt').limit(5); const revenue = await Order.aggregate([ { $match: { status: { $in: ['delivered', 'shipped'] } } }, { $group: { _id: null, total: { $sum: '$total' } } } ]); const today = new Date(); today.setHours(0, 0, 0, 0); const todayOrders = await Order.countDocuments({ createdAt: { $gte: today } }); const lowStock = await Product.countDocuments({ stock: { $lt: 5 } }); res.json({ stats: { totalOrders, pendingOrders, confirmedOrders, processingOrders, shippedOrders, deliveredOrders, cancelledOrders, totalProducts, totalReviews, pendingReviews, revenue: revenue[0]?.total || 0, todayOrders, lowStock }, recentOrders }); } catch (error) { res.status(500).json({ error: error.message }); }
});
